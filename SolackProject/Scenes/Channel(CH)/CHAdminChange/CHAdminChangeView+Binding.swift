//
//  CHAdminChangeView+Binding.swift
//  SolackProject
//
...
extension CHAdminChangeView{
    func dialogAndCloseBind(reactor: CHAdminChangeReactor){
        reactor.state.map{$0.chManagerDialog}.delay(.microseconds(100), scheduler: MainScheduler.instance).bind(with: self) { owner, dialogType in
            guard let dialogType else {return }
            switch dialogType{
            case .checkAlert(userNick: let nick, userID: let userID):
                let title = "\(nick) 님을 관리자로 지정하시겠습니까?"
                let desc = "채널 관리자는 다음과 같은 권한이 있습니다."
                let infos = ["채널 이름 또는 설명 변경","채널 삭제"]
            ...
            case .nonMember:
                let vc = SolackAlertVC(title: "채널 관리자 변경 불가", description: "채널 멤버가 없어 관리자 변경을 할 수 없습니다.", cancelTitle: "확인", cancel: {
                    Task{
